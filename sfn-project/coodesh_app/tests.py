...
class SFNArticlesUpdateTestCase(APITestCase):
    ...
    def test_update_product(self):
        ...
        response = self.client.put(
            'http://127.0.0.1:8000/articles/{}/'.format(sfnarticles.my_id),
            {
                "title": "New_title"
            }
        )
        print("response.data", response.data)
        sfnarticles_updated =  SFNArticles.objects.get(my_id=sfnarticles.my_id)
        self.assertEqual(sfnarticles_updated.title, 'New_title')
